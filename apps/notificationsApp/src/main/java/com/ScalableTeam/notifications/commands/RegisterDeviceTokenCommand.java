...
@Slf4j
@Service
public class RegisterDeviceTokenCommand implements Command {
    ...
    public Integer execute(Object body) throws Exception {
        ...
        notificationsRepository.registerDeviceToken(deviceToken);
        return 200;
    }

    @RabbitListener(queues = "${mq.queues.request.notifications.registerDeviceToken}")
    public Integer onMessageReceived(DeviceTokenRequest deviceTokenRequest, Message message) throws Exception {
        String indicator = generalConfig.getCommands().get("registerDeviceToken");
        String correlationId = message.getMessageProperties().getCorrelationId();
        log.info(indicator + "Service::Request::CorrelationId: {}", correlationId);
        return execute(deviceTokenRequest);
    }

    @RabbitListener(queues = "${mq.queues.response.notifications.registerDeviceToken}")
